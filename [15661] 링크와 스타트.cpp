...
// 스타트 그룹 능력치
void getStart(int cnt, int select)
{
    // 2개를 모두 선택했다면
    if(cnt == 3) {
...
// 링크 그룹 능력치
void getLink(int cnt, int select)
{
    // 2개를 모두 선택했다면
    if(cnt == 3) {
...
void dfs(int cnt, int select)
{
    ...
    if(cnt == select+1) {
        ...
        getStart(1, select); getLink(1, select); // 각 그룹 능력치 계산
